  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <main className="max-w-5xl mx-auto px-6 py-10">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white">Explore Athletes</h1>
        ...
        {athletes.length === 0 ? (
          <div className="text-center py-20 text-gray-500">
            ...
            <Link href="/sign-up" className="mt-4 inline-block bg-blue-600 text-white px-6 py-2.5 rounded-lg font-semibold hover:bg-blue-500 transition">
              Join as Athlete
            </Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {athletes.map((athlete) => (
              <Link
                ...
                className="group bg-gray-900 rounded-2xl border border-white/10 hover:border-blue-500/50 transition p-6 flex flex-col"
              >
                {/* Avatar */}
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <div>
                    <h2 className="font-bold text-white">{athlete.name ?? "Athlete"}</h2>
                    <p className="text-xs text-blue-400 font-medium">WAKO Athlete</p>
                  </div>
                </div>
                ...
                {/* Stats */}
                <div className="flex gap-4 text-sm text-gray-500 mt-auto pt-4 border-t border-white/10">
                  <span>
                    <strong className="text-white">{athlete.courses.length}</strong> courses
                  </span>
                  <span>
                    <strong className="text-white">{athlete.posts.length}</strong> posts
                  </span>
                  <span>
                    <strong className="text-white">{athlete._count.subscribers}</strong> subscribers
                  </span>
                </div>
              </Link>
            ))}
